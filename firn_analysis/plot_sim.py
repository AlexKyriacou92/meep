...
fig, ax = pl.subplots()
ax.set_aspect('auto')
pl.imshow(10*np.log10(abs(E_sq)), vmin=-80, vmax=0, aspect='auto', cmap='hot',extent=[0, R_ice, -Z_ice, H_air])
pl.axis('on')
...
fig, ax = pl.subplots()
ax.set_aspect('auto')
pl.imshow(epsilon_r, aspect='auto', cmap=cmr.arctic,extent=[0, R_ice, Z_ice, H_air])
pl.axis('on')
#plot_title = "dipole_"+name+"_d_"+str(depth)
#pl.title(plot_title)
ax.set_xlabel("radial direction (m)")
